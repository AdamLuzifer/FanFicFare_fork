...
class AuthorTodayAdapter(BaseSiteAdapter):
        ...
        try:
            ...
            try:
                ...
                if 'isSuccessful' in json_response and json_response['isSuccessful']:
                    ...
                    user_id_match = re.search(r'window\.app\s*=\s*{\s*.*?userId\s*:\s*[\'"]?(\d+)[\'"]?', response.text, re.DOTALL)
                    if user_id_match:
    ...
    def decrypt_chapter_text(self, data, secret):
        ...
        """
        try:
            if not data or 'text' not in data or not secret:
                logger.error('Invalid encrypted data or reader secret')
                return ''
            
            # Match JavaScript exactly:
            # let ss = secret.split("").reverse().join("") + "@_@" + (app.userId || "");
            ss = secret[::-1] + "@_@" + (self.user_id or "")
            slen = len(ss)
            text = data['text']
            clen = len(text)
            
            # JavaScript: result.push(String.fromCharCode(chapter.data.text.charCodeAt(pos) ^ ss.charCodeAt(pos % slen)))
            result = []
            for pos in range(clen):
                result.append(chr(ord(text[pos]) ^ ord(ss[pos % slen])))
    ...
    def getChapterText(self, url):
        ...
        try:
            ...
            # Look for reader-secret in headers first (case-insensitive)
            reader_secret = None
            ...
            # Then try response data if not found in headers
            if not reader_secret:
                if 'readerSecret' in json_data:
                    reader_secret = json_data['readerSecret']
                elif 'data' in json_data and 'readerSecret' in json_data['data']:
            ...
            # Decrypt chapter content using JavaScript-compatible method
            decrypted_text = self.decrypt_chapter_text(json_data['data'], reader_secret)
